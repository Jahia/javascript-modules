...
Module layout in 2.0.0 — one migration per file, name = run-once identity:
...
registerMigration(
  ...
  ({ migrate }) => {
    migrate.removePropertyValues({ nodeType: "acme:product", property: "color" });
  },
);
```
...
```ts
// src/migrations/2.0.0-02-backfill-theme.server.ts
import { registerMigration } from "@jahia/javascript-modules-library";
...
```ts
// src/migrations/2.0.0-03-convert-priority.server.ts
import { registerMigration } from "@jahia/javascript-modules-library";
...
```ts
// src/migrations/2.0.0-04-drop-legacy-badge.server.ts
import { registerMigration } from "@jahia/javascript-modules-library";
...
Guard rails: the type must belong to **this module** (own-definitions only — cross-module surgery like the visibility/jcontent scripts is out of scope); with the default `"fail"`, the migration is marked `.failed` and content is left untouched if instances still exist.

## 5. U5 — rename a definition + rebind existing items

```ts
// src/migrations/2.0.0-05-rename-old-teaser.server.ts
import { registerMigration } from "@jahia/javascript-modules-library";
...
registerMigration(
  ...
  ({ migrate }) => {
    migrate.changeNodeType({
      from: "acme:oldTeaser",
      to: "acme:teaser", // must exist in the 2.0.0 CND (registered before migrations run)
...
## 6. Escape hatch — imperative migration (templates-system-style repair)

```ts
// src/migrations/2.0.0-06-clean-catalog-root-title.server.ts
import { registerMigration } from "@jahia/javascript-modules-library";
...
For bulk imperative work, `jcr.forEachNode({ query: "SELECT * FROM [acme:product]", workspaces: ["default", "live"] }, (node) => { … })` provides the same batching/save/progress engine the `migrate.*` helpers use.

## 7. The API contract (what autocomplete shows)

Trimmed `.d.ts` — the implementation target. **Migrations are synchronous** (no promises), consistent with actions and the GraalJS execution model; a returned thenable fails fast with a clear error. All underlying JCR calls block anyway, so `async` would buy nothing.

```ts
/**
 * Registers a migration: a run-once script executed on the processing server when a new
...
interface MigrationDeclaration {
   ...
   * NEVER rename or reorder after release — ship a new migration instead.
...
interface MigrationContext {
  ...
  jcr: MigrationJcr;
  /** SLF4J-backed logger (org.jahia.modules.javascript.modules.engine.migrations.<module>). */
  log: Logger;
  /** True in dry-run mode (the DEFAULT for `jahia migrations run`) — helpers report instead of saving. */
  dryRun: boolean;
...
Dev/staging servers set `migrations.autoRun: false` in the engine config (new setting); production keeps the default `true` (migrations run automatically at module start, exactly like Groovy patches). The CLI is safe by default: **`run` is a dry-run unless you pass `--no-dry-run`** (Sanity-inspired), and applying asks for confirmation (`--yes` for CI).

```console
$ npx jahia migrations create backfill-theme       # scaffolding: computes the next NN
Created src/migrations/2.0.0-02-backfill-theme.server.ts

$ yarn watch                       # build + deploy 2.0.0 as usual
# engine log: [migrations] acme-catalog 2.0.0: 6 pending migrations (autoRun disabled — use CLI)

$ npx jahia migrations status
MODULE         NAME                                  STATUS
...
$ npx jahia migrations run                          # dry-run by default — nothing is saved
...
dry-run: 6 migrations, 0 errors — pass --no-dry-run to apply

$ npx jahia migrations run --no-dry-run
Apply 6 migrations to acme-catalog on http://localhost:8080? [y/N] y
…
2.0.0-06-clean-catalog-root-title   .installed (0.1s)
6 installed, 0 failed, 0 skipped

$ npx jahia migrations run --no-dry-run --yes       # rerun-safety: identity is the name
nothing pending

# iterate on one script against disposable content:
$ npx jahia migrations reset --name 2.0.0-03-convert-priority
$ npx jahia migrations run --name 2.0.0-03-convert-priority        # dry-run again
```

Failure in production (autoRun on): Groovy parity — the module still starts, the failing migration is recorded `.failed`, remaining migrations for that module are halted and stay `pending`:

```
ERROR [migrations] acme-catalog 2.0.0-03-convert-priority failed: <stack> — recorded .failed;
      halting 3 remaining migrations for acme-catalog
```

Recovery beats Groovy (no hand-editing `j:bundlesScripts`): ship the fixed script (same name), then `npx jahia migrations reset --name … && npx jahia migrations run` (or the GraphQL mutations, admin-gated).

## 9. Testing your migrations (Q5 — the two levels explained)
...
1. `yarn pack` the module twice — v1.0.0 (old CND, no migrations) and v2.0.0.
2. Start Jahia (docker), provision v1, create fixture content (GraphQL).
3. Deploy v2, wait for migration status via GraphQL.
...
A pure-JS `MigrationContext` stand-in: `migrate.*`/`jcr.*` record their calls instead of touching a repository; your `convert`/`value` callbacks run for real. Tests assert structure and callback logic in milliseconds, no docker:

```ts
const { context, calls } = createMigrationTestContext();
runMigration("2.0.0-03-convert-priority", context);
