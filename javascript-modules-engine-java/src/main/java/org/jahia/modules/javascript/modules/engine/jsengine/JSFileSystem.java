...
/**
 * This class defines a virtual filesystem used to resolve <code>import statements</code>.
 *
...
public class JSFileSystem implements FileSystem {
    ...
    /**
     * parsePath is called on non-path imports (e.g. <code>import "react"</code> but not <code>import
     * "./react.js"</code>). It's the first step in the resolution process.
     *
     ...
     *
     * For instance, it might receive <code>/META-INF/js/libs/react-i18next.js</code> (stringified output of the method)
     * or <code>/usr/local/graalvm/languages/js</code> (internal import) or even worse
     * <code>d:\jdk17\languages\js</code> on Windows.
     *
     * This is safe-guarded by this <code>path.startsWith</code> check until someone comes with a better solution.
     */
    @Override
     ...
     * after they were resolved, like <code>/META-INF/js/libs/I18nextProvider-BnrOfLKR.js</code>. All paths are valid at
    ...
    /**
     * This method is called for imports with a protocol (e.g. <code>import "http://example.com"</code> or <code>import
     * "node:http"</code>). We might be able to build cool things in the future with this!
     */
    @Override
