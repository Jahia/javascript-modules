 ...
 *
 * <p>This class extends ServletOutputStream and provides a functional implementation that writes data to an internal
 ...
 * processed.</p>
 *
 * <p>Key features of this mock implementation:</p> <ul> <li>Captures all written data in an internal
 * {@link ByteArrayOutputStream} buffer</li> <li>Supports configurable character encoding for string conversion</li>
 * <li>Defaults to UTF-8 encoding if no encoding is specified</li> <li>Always reports as ready for writing (
 * {@link #isReady()} returns true)</li> <li>Provides a {@link #getContent()} method to retrieve buffered content as a
 * string</li> <li>Write listener operations are no-ops (not supported)</li> </ul>
 *
 * <p>The primary use case is to capture output from JavaScript components during server-side rendering, allowing the
 * rendered HTML content to be retrieved and used within the Jahia framework. The captured content can be accessed via
 * the {@link #getContent()} method.</p>
 *
