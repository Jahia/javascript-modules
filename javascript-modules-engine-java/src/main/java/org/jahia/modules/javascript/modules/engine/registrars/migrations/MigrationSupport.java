...
/**
 * Java support object handed to a JavaScript migration when it is executed. It carries the
 * per-migration logger, the dry-run flag, module metadata, and the sanctioned definition
 * operations a migration is allowed to perform (restricted to node types owned by the module,
 * i.e. whose {@code systemId} is the module's symbolic name).
 *
 * <p>The JS side (the {@code registerMigration} wrapper in the library) builds the idiomatic
 * {@code MigrationContext} from this object — keep both shapes in sync.
 */
public class MigrationSupport {

    private static final String LOGGER_PREFIX = "org.jahia.modules.javascript.modules.engine.migrations.";

    private final Bundle bundle;
    private final boolean dryRun;

    public MigrationSupport(Bundle bundle, boolean dryRun) {
        this.bundle = bundle;
        this.dryRun = dryRun;
    }

    /** Logger dedicated to one migration, named {@code …migrations.<module>.<migrationName>}. */
    public Logger getLogger(String migrationName) {
        return LoggerFactory.getLogger(LOGGER_PREFIX + bundle.getSymbolicName() + "." + migrationName);
    }
     ...
     * out of scope for JS migrations.
     *
    ...
    public void unregisterNodeType(String name) {
        ...
        if (!bundle.getSymbolicName().equals(type.getSystemId())) {
            throw new IllegalArgumentException("Node type " + name + " is owned by '" + type.getSystemId()
                    ...
                    + "') — migrations may only remove their own definitions");
        }
        if (dryRun) {
