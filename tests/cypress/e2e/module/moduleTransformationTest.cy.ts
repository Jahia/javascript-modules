describe("Check that the Javascript module has been transformed properly and has the proper header values", () => {
  it("Check the transformed module headers", () => {
    cy.executeGroovy("groovy/getBundleHeaders.groovy", {
      ...
      expect(result).to.contain("Bundle-Name: JS Modules Engine Test Module (javascript module)");
      ...
      expect(result).to.contain(
        'moduleIdentifier="JS Modules Engine Test Module (javascript module)"',
      );
      ...
      expect(result).to.contain(
        'com.jahia.services.content;filter:="(nodetypes=jmix:accessControllableContent)"',
      );
      expect(result).to.contain(
        'com.jahia.services.content;filter:="(nodetypes=jmix:bindedComponent)"',
      );
      expect(result).to.contain(
        'com.jahia.services.content;filter:="(nodetypes=jmix:droppableContent)"',
      );
      ...
      expect(result).to.contain(
        'com.jahia.modules.dependencies;filter:="(moduleIdentifier=default)"',
      );
      expect(result).to.contain(
        'com.jahia.modules.dependencies;filter:="(moduleIdentifier=legacy-default-components)"',
      );
      expect(result).to.contain(
