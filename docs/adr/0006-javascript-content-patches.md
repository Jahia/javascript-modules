...
- `registerContentPatch(declaration, run)` (library) stores a raw `execute` adapter under registry type `content-patch`; `ContentPatchRegistrar` — the once-per-start `Registrar` seam introduced for server extensions — sorts entries by name, filters against recorded statuses, and executes pending ones inside `doWithContext`, handing each a `ContentPatchSupport` Java object (logger, dry-run flag, module metadata) that hands out the shared operations engine.
- The operations themselves (the `patch.*` transformations, the `forEachNode` batching engine, i18n handling, definition guard rails, dry-run) are implemented **once, in Java**, in the `…engine.contentpatches` package — the engine bundle's exported API, free of GraalVM and engine-internal types. The TypeScript surface is a thin typed façade over it, and Groovy patch scripts / Java modules consume the identical engine through the `ContentPatchService` OSGi service. A JS-only implementation would have forked this language-neutral know-how (batching, translation subnodes, property-carrying across retypes) per language and widened the JS/Java gap. Callbacks are plain functional interfaces, which Groovy closures and JS functions coerce to natively; guest exceptions cross the engine untouched so `skip()` detection survives the host boundary.
...
- Good: one operations engine serves TypeScript, Groovy and Java — semantics, logging and future tooling cannot drift between languages, and parity is asserted operation-by-operation by twin Cypress suites. The exported package only depends on core-exported types, keeping it movable to core later without redesign.
