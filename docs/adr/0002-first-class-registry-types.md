...
- The registry keys entries as `type-key`. With a shared `service` type, an action named `foo` and a choicelist initializer named `foo` collide and `Registry.add` throws. For actions and choicelists the key _is_ the platform-visible name, so cross-kind collisions are a real hazard, not a theoretical one.
