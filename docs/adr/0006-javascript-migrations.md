# Run JavaScript migrations through a dedicated registrar backed by core's patch status store
...
* Migrations must see the module's **new** definitions (core registers CND at `RESOLVED`, before `STARTED`).
* Failures must never prevent a module from starting (Groovy parity).
* No Jahia core changes.

## Considered Options

1. **A dedicated `Registrar` (`MigrationRegistrar`) executing registry entries of type `migration` at `BundleEvent.STARTED`, recording results through `BundleInfoJcrHelper` into the same status store as Groovy patches.**
...
* `registerMigration(declaration, run)` (library) stores a raw `execute` adapter under registry type `migration`; `MigrationRegistrar` — the once-per-start `Registrar` seam introduced for server extensions — sorts entries by name, filters against recorded statuses, and executes pending ones inside `doWithContext`, handing each a `MigrationSupport` Java object (logger, dry-run flag, module metadata, owned-definitions operations).
* Statuses (`.installed` / `.skipped` / `.failed`, all terminal) are recorded under pseudo-paths `/javascript/migrations/<name>` via the public, OSGi-exported `BundleInfoJcrHelper.get/storeModuleScriptStatus` — one audit trail for Groovy and JS, identical semantics, zero core changes. The store is deliberately NOT written through `Patcher.executeScripts`, which would gate 4-segment-version-prefixed names against the *platform* version.
* Processing-server-only; a recorded failure is a **persistent barrier** — migrations ordered after it stay held across restarts until the record is cleared — but the module always starts; an `autoRun=false` configuration (PID `…engine.migrations`) defers execution on dev/staging servers.
* Migrations are synchronous (like actions — no promises in the GraalVM execution model); the wrapper fails fast on returned thenables.
...
* Good: the typed JCR mutation surface added for migrations (java-ts-bind `methodWhitelist`: `setProperty`, `save`, `remove`, `setPrimaryType`, …) also fixes untyped mutation for actions.
* Bad: no checksum on names — an edited-but-same-name migration silently does not re-run (Groovy parity; tooling may surface hash mismatches later).
* Bad: migrations block the bundle-event thread (Groovy parity); built-in batching bounds the cost, a background-execution option remains future work.
