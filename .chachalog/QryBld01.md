...
`getNodesByJCRQuery` and `useJCRQuery` accept a built query, which carries its own limit and offset. The `useJCRQuery` form that takes a statement and no limit is deprecated, because it returns every node the query matches.
