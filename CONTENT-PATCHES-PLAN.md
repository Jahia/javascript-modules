...
* **Ops consistency** — status lands in the *same* JCR store (`/module-management` → `j:bundlesScripts`) as Groovy module patches, under a distinguishable pseudo-path prefix (`/javascript/content-patches/…` vs `/META-INF/patches/…`). One place to audit; cluster-shared; identical terminal semantics.
...
2. *Separate build entry per content patch* (e.g. `*.patch.ts` → own dist file + new bundle header) — more moving parts (vite-plugin, transformer, header parsing) for no v1 benefit. The registry already gives us enumeration + lazy execution. Can be added later if init-script size becomes a concern.
...
* **Unit (vitest, no Jahia)**: `convert`/`value` callbacks are pure and trivially testable. Stretch: `@jahia/javascript-modules-library/testing` fake that records `patch.*` calls for asserting content-patch *structure*.
