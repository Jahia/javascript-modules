...
- `environment`: raised by the construct whose cost the condition changes, and never on its own, so a query with nothing to report returns `[]`. An ordering raises it for `useNativeSort=false`, and an offset raises it for the extra providers. Render mode and session locale are not raised per query, because they apply to nearly every query; the guide covers both. An ordering is a common shape, so an ordered query always carries this one entry. The trigger is kept, because the condition it names is real and the model cannot see it, and the guide says that this entry is expected rather than a problem.

`build({ strict: true })` throws on `none`, and `executeQuery` throws `UNSUPPORTED` on `none`. A `none` finding marked `conditional` is reported and not thrown on, because refusing it would take `not()` and `upper()` away from every caller for a condition that holds in few queries.

An earlier revision of this section said that no finding exists for a `Slow` construct, because the name is the warning. That holds for every construct whose cost stays proportional to the page, and it was reversed for the two that do not. A join and an unbounded execution both drop the bound on the search itself, so each carries a `full-scan` finding: `full-scan at=source` for every join, whatever its type, and `full-scan at=execution.limit :: This query returns every matching node.` for `unboundedSlow()`.
...
`src/query/execute.ts` is internal in v1, and the public seams are `getNodesByJCRQuery` and `useJCRQuery`. `executeQuery(session, input)` takes either `{ statement: string; limit: number; offset: number }` from a string seam or an `Executable`. It is the sole caller of `setLimit` and `setOffset`: it calls `setLimit(limit)` when `limit >= 0`, `setOffset(offset)` when `offset > 0`, and then `execute()`.

It is also where the limit rule holds at run time. A builder that reaches it with no limit, from neither the query nor the call, throws `UNSUPPORTED` before the first host call, because the type state does not reach a JavaScript caller or an `as` cast. The deprecated string overload of `useJCRQuery` applies the same rule: it used to pass `-1` for a caller who passed nothing, and it now throws instead, so `-1` is written at the call site by whoever wants it. Two routes stay outside the rule and both leave the library: `toQOM` hands back the host query object, whose `execute()` runs with no limit and no `diagnose()`, and a query built through the session's own query manager never enters the library. A runtime guard on `toQOM` is not possible without breaking its documented use, which is reading `getStatement()` on a query that needs no limit, so the guide and the README say what each route costs instead. A string input goes through `createQuery(statement, "JCR-SQL2")`, as `getNodesByJCRQuery.ts:29` does today, and an `Executable` goes through `toQOM`. No layer slices in JavaScript.
...
7. Unbounded queries. Romain chose "limit() required, typed (Recommended)". A built query cannot be executed until `.limit(n)` was called. The builder's type state enforces it at compile time, and `executeQuery` enforces the same rule at run time, so the guarantee is not restricted to TypeScript callers. The explicit escape hatch is `.unboundedSlow()`, and `offset` stays optional.
