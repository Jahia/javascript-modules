...
interface MenuConfig {
  ...
  workspace: string;
  /**
   * The name of the menu, used to match with the displayInMenuName property to see if this entry
   * should be displayed in this specified menu
   */
  menuName: string;
 ...
 * @param navMenuLevel The current depth of the menu (1 for the root, 2 for the children of the
 *   root, etc.)
...
const buildMenu = (
  ...
  if (node) {
    ...
    for (let index = 0; index < children.length; index++) {
      ...
      const inpath =
        config.mainResourceNode.getPath() === itemPath ||
        config.mainResourceNode.getPath().startsWith(itemPath + "/");
      let selected = false;
      ...
      if (menuItem.hasProperty("j:displayInMenuName")) {
        correctType = false;
        menuItem
          .getProperty("j:displayInMenuName")
          .getValues()
          .forEach((displayMenuValue) => {
      ...
      if (!referenceIsBroken && correctType && (config.startLevelValue < navMenuLevel || inpath)) {
        if (navMenuLevel < config.maxDepth) {
          const menuItemChildren = getMenuItemsChildren(config.workspace, menuItem.getPath(), [
            "jmix:navMenuItem",
          ]);
