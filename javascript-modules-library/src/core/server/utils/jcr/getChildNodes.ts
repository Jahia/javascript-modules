 ...
 * @param limit the maximum number of nodes to return (-1 to return all nodes, but be careful with this as it can be very slow and memory consuming, it's better to use a reasonable limit and use pagination if needed)
 * @param offset the offset to start from
 * @param filter A function to filter the nodes to be returned.
 * @returns An array of child nodes.
 */
export function getChildNodes(node: JCRNodeWrapper, limit: number, offset = 0, filter: ((node: Node) => boolean) | undefined = undefined): Node[] {
    const result: Node[] = [];
