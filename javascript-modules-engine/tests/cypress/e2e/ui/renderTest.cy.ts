...
describe('Test on render and createContentButtons helpers', () => {
    ...
    it(`${pageName}: should render javascriptExample:test JSON node in config: OPTION with view: sub`, function () {
        cy.login()
        cy.visit(`/cms/render/default/en/sites/javascriptTestSite/home/${pageName}.html`)
        cy.get('div[data-testid="component-react-json-node-option"]').should('contain', 'prop1 value it is')
        cy.get('unwanteddiv').should('not.exist')
        cy.logout()
    })
    ...
    it(`${pageName}: should render node with parameters passed to render`, function () {
        cy.login()
        cy.visit(`/cms/render/default/en/sites/javascriptTestSite/home/${pageName}.html`)
        cy.get('div[data-testid="component-react-node-with-parameters"] div[data-testid="renderParam-string1"]').should(
            'contain',
            'stringParam=stringValue',
        )
        cy.get('div[data-testid="component-react-node-with-parameters"] div[data-testid="renderParam-string2"]').should(
            'contain',
            'stringParam2=stringValue2',
        )
        cy.get(
