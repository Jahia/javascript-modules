...
describe('Test on currentContent injected data', () => {
    ...
    it(`${pageName}: Check currentContent injected JSON node in current view`, function () {
        ...
        cy.get('div[data-testid="currentContent_properties_jcr:title"]').should('contain', 'test component')
        ...
        cy.get('div[data-testid="currentContent_nodeType"]').should('contain', 'javascriptExample:test')
        cy.logout()
    })
})
