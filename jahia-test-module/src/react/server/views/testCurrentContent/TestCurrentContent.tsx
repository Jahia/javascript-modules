...
jahiaComponent(
  {
    nodeType: "javascriptExample:testCurrentContent",
    name: "default",
    displayName: "test currentContent",
    componentType: "view",
  },
  (_, { currentResource, currentNode, mainNode }) => {
    const mixins = currentNode
      .getMixinNodeTypes()
      .map((e) => {
        return e.getName();
      })
      .join(",");
    const propMultiple = currentNode
      .getProperty("propMultiple")
      .getValues()
      .map((e) => {
        return e.getString();
      })
    ...
    return (
      <>
        ...
        <div data-testid="currentContent_properties_jcr:title">
          jcr:title : {currentNode.getPropertyAsString("jcr:title")}
        </div>
        <div data-testid="currentContent_properties_prop1">
          prop1 : {currentNode.getPropertyAsString("prop1")}
        </div>
        <div data-testid="currentContent_properties_propMultiple">
          propMultiple : {propMultiple}
        </div>
        <div data-testid="currentContent_properties_propI18n">
          propI18n : {currentNode.getPropertyAsString("propI18n")}
        </div>
        ...
        <div data-testid="currentContent_nodeType">
          nodeType : {currentNode.getPrimaryNodeTypeName()}
        </div>
